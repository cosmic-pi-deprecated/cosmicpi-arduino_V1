...
int32_t LPS::readPressureRaw(void)
{
  ...
  Wire1.write(PRESS_OUT_XL | (1 << 7));
  Wire1.endTransmission();
  Wire1.requestFrom(address, (byte)3);

  while (Wire1.available() < 3);

  uint8_t pxl = Wire1.read();
  uint8_t pl = Wire1.read();
  uint8_t ph = Wire1.read();
...
int16_t LPS::readTemperatureRaw(void)
{
  Wire1.beginTransmission(address);
  // assert MSB to enable register address auto-increment
  Wire1.write(TEMP_OUT_L | (1 << 7));
  Wire1.endTransmission();
  Wire1.requestFrom(address, (byte)2);
